...
```bash
serverless-hands-on$ sam logs -n HelloEfsFunction --stack-name serverless-hands-on --tail
```
...
```bash
serverless-hands-on$ pip install -r tests/requirements.txt --user
# unit test
serverless-hands-on$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
serverless-hands-on$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name serverless-hands-on
```
